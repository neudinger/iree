...
// RUN:   --iree-gpu-test-target=sm_89 --iree-hal-dump-executable-binaries-to=- %s \
// RUN:   2>&1 | FileCheck %s --check-prefix=SM89
// RUN: iree-opt --split-input-file --iree-hal-transformation-pipeline \
// RUN:   --iree-gpu-test-target=sm_90 --iree-hal-dump-executable-binaries-to=- %s \
// RUN:   2>&1 | FileCheck %s --check-prefix=SM90
// RUN: iree-opt --split-input-file --iree-hal-transformation-pipeline \
// RUN:   --iree-gpu-test-target=sm_100 --iree-hal-dump-executable-binaries-to=- %s \
// RUN:   2>&1 | FileCheck %s --check-prefix=SM100
// RUN: iree-opt --split-input-file --iree-hal-transformation-pipeline \
// RUN:   --iree-gpu-test-target=sm_120 --iree-hal-dump-executable-binaries-to=- %s \
// RUN:   2>&1 | FileCheck %s --check-prefix=SM120
